...
function GameContainer() {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (
        event.data &&
        (event.data.type === "GAME_OVER" || event.data.score !== undefined)
      ) {
        const scoreValue = Number(event.data.score ?? event.data.value ?? 0);

        if (scoreValue > 0 && !gameOver && !processing) {
          processGameOver(scoreValue);
        }
      }
    };
  ...
  const processGameOver = async (score: number) => {
    ...
    try {
      // IMPORTANT:
      // Send the ACTUAL GAME SCORE to the server.
      // The database calculates the legitimate reward.
      const earned = await addCash(score, "playnpayday_game");
      ...
      setReward(0);
      toast.error("Unable to credit game reward.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleDouble = async () => {
    if (doubled || reward <= 0 || processing) return;
    ...
    try {
      ...
      toast.error("Unable to double reward.");
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="h-screen w-screen bg-black flex flex-col overflow-hidden relative">
      ...
      {loading && !gameOver && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black text-white z-40">
          ...
          <p className="mt-6 font-black italic uppercase tracking-[0.3em] text-sm animate-pulse">
            Loading Table...
          </p>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-6 text-white text-center">
          <div className="space-y-8 w-full max-w-md animate-in fade-in zoom-in duration-300">
            ...
            <div className="space-y-2 text-center">
              <h2 className="text-6xl font-black uppercase italic tracking-tighter text-yellow-400">
                FINISH!
              </h2>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-10 shadow-2xl">
              ...
              <p className="text-7xl font-black tabular-nums tracking-tighter text-white italic">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button
                className="bg-white/5 border border-white/10 rounded-2xl p-5 flex flex-col items-center gap-1 active:bg-white/10 transition-colors"
              >
            ...
            <div className="flex flex-col gap-4 pt-4">
              <button
                ...
                className="w-full bg-white text-black py-4 rounded-xl font-black uppercase tracking-widest text-sm"
              >
                New Ball
              </button>
              ...
              <button
                onClick={() => navigate({ to: "/" })}
                className="text-white/40 font-bold uppercase text-[10px] tracking-[.3em]"
              >
                Return to Lobby
              </button>
            </div>
